        ...
        >
          ...
          >
            ...
            <text
              ...
              :class="[
                item1.disabled && 'u-calendar-month__days__day__select__buttom-info--disabled'
              ]"
              :style="[textStyle(item1)]"
              >{{ getBottomInfo(index, index1, item1) }}</text
            >
            <text v-if="item1.dot" class="u-calendar-month__days__day__select__dot" />
          </view>
        </view>
      </view>
    </view>
    ...
    dayStyle(index1, index2, item) {
      return (index1, index2, item) => {
        ...
        const dayWidth = Number(
          Number.parseFloat(this.width / 7)
            .toFixed(3)
            .slice(0, -1)
        )
        // 得出每个日期的宽度
      ...
      if (this.mode === 'single') {
        ...
        if (selected.length === 0 || selected.length >= 2) {
          ...
          if (dayjs(date).isBefore(existsDate)) {
            ...
            if (
              dayjs(dayjs(date).subtract(this.maxRange, 'day')).isAfter(dayjs(selected[0])) &&
